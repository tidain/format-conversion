# 格式转换工具 安装程序构建脚本
# 流程：先复用 portable-package.ps1 生成免安装版，再用 Inno Setup 压成单文件安装包
# 输出：release\FormatConverter-setup-<版本>.exe（中文向导、可选安装目录、开始菜单/桌面快捷方式、可卸载）
...
& $iscc "/DAppVersion=$AppVersion" (Join-Path $root "installer\FormatConverter.iss")
if ($LASTEXITCODE -ne 0) { throw "ISCC failed with exit $LASTEXITCODE" }

$outFile = Join-Path $root "release\FormatConverter-setup-$AppVersion.exe"
