...
本软件依赖以下第三方库和工具：
- **Kotlin**、**JetBrains Compose Multiplatform**（用于构建现代化桌面界面）
- **JNA**（用于 Windows 系统底层交互）
- **FFmpeg**（用于视频、音频、图片等格式转换，**仅本地调用**）
- **LibreOffice**（用于 PDF ↔ DOCX 文档高保真转换，**仅本地调用**）
- **Windows 系统内置的解压缩功能**（用于 ZIP、RAR 等压缩包的处理，仅使用系统原生接口）

这些依赖仅用于实现软件功能，不会主动收集或上传您的数据。
